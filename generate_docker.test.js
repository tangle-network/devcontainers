...
function testDeduplication() {
    ...
    fs.rmSync(INFRA_DIR, { recursive: true });
    fs.mkdirSync(INFRA_DIR, { recursive: true });
    ...
    if (!fs.existsSync(file2)) {
        console.error('❌ FAILED: Expected ethereum_polygon_zksync.Dockerfile to be generated');
        cleanup();
        process.exit(1);
    }
...
function testDifferentOrders() {
    ...
    for (const testCase of testCases) {
        ...
        const contents = [];
        const expectedFilename = testCase[0].split('_').sort().join('_') + '.Dockerfile';
        ...
        for (const spec of testCase) {
            ...
            if (files.length !== 1) {
                console.error(`❌ FAILED: Expected 1 file, got ${files.length}`);
                cleanup();
                process.exit(1);
            }
            
            if (files[0] !== expectedFilename) {
                console.error(`❌ FAILED: Expected ${expectedFilename}, got ${files[0]}`);
                cleanup();
                process.exit(1);
            }
            
            const content = fs.readFileSync(path.join(INFRA_DIR, files[0]), 'utf8');
            contents.push(content);
        }
        
        const allSame = contents.every(c => c === contents[0]);
        if (!allSame) {
            console.error(`❌ FAILED: Contents differ for ${testCase.join(', ')}`);
            cleanup();
            process.exit(1);
        }
        
        console.log(`✅ PASSED: ${testCase.join(' = ')} -> ${expectedFilename}`);
...
function testAlphabeticalSorting() {
    ...
    if (!content.includes('LABEL description="Combined: ethereum, polygon, zksync"')) {
        console.error('❌ FAILED: Label should contain alphabetically sorted project names');
        console.error('Content:', content);
        cleanup();
        process.exit(1);
    }
    
    console.log('✅ PASSED: Labels contain alphabetically sorted project names');
