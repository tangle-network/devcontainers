...
USER root
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends libgmp-dev && \
    rm -rf /var/lib/apt/lists/*

USER root
RUN export HOME=/root && curl --proto '=https' --tlsv1.2 -sSf https://docs.swmansion.com/scarb/install.sh | sh && \
